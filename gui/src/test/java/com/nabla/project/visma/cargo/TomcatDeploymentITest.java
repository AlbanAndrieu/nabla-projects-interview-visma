...
public class TomcatDeploymentITest {
  ...
  // private final static String pricerServiceBase = "http://localhost:" + applicationPort +
  // "/scripting/services"; private final static String WEBAPI_BASE = "http://localhost:" +
  // WEBAPI_PORT + "/api";
  ...
  private static final String TOMCAT8_URL = "http://archive.apache.org/dist/tomcat/tomcat-8/v"
      + TOMCAT8_VERSION + "/bin/apache-tomcat-" + TOMCAT8_VERSION + ".zip";
  // private final static String TOMCAT6_VERSION = "6.0.32";
  // private static final String TOMCAT6_URL = "http://archive.apache.org/dist/tomcat/tomcat-6/v" +
  // TOMCAT6_VERSION + "/bin/apache-tomcat-" + TOMCAT6_VERSION + ".zip";
  ...
  public static void setup() throws Exception {
    ...
    WEBAPI_CONTAINER = (InstalledLocalContainer) new DefaultContainerFactory().createContainer(
    ...
    int containerTimeout = (containerTimeoutProperty != null)
        ? Integer.valueOf(containerTimeoutProperty)
        : 5 * 60 * 1000;
    ...
    properties.put("application.configuration.file",
        Paths.get("src/test/resources/application.properties").toAbsolutePath().toString());
   ...
   * ClientResponse docResponse = webClient.resource(format("%s/storage/scripts/%s/",
   * pricerServiceBase, scriptId)).get(ClientResponse.class); return
  ...
  protected void test(final String scriptId, String testDeal) throws Exception {
    ...
    // WebResource contractDefResource = webClient.resource(format("%s/storage/fields/%s/",
    // pricerServiceBase, scriptId)); String scriptDetails = loadScriptDetails(scriptId);
    // ClientResponse fieldResponse =
    // contractDefResource.type("application/json").post(ClientResponse.class, scriptDetails);

    // assertEquals(format("Error getting contract from script %s", scriptId), 200,
    // fieldResponse.getStatus());
    ...
    // WebResource contractPublishResource = webClient.resource(format("%s/storage/publish/%s/",
    // pricerServiceBase, scriptId)); ClientResponse publishResponse =
    // contractPublishResource.type("application/json").post(ClientResponse.class, fields);

    // assertEquals(format("Error publishing contract for script %s", scriptId), 200,
    ...
    // ClientResponse docResponse = webClient.resource(format("%s/doc/1.0/contract/%s/",
    // WEBAPI_BASE, scriptId)).get(ClientResponse.class); assertEquals(format("Error getting
    // documentation for script %s", scriptId), 200, docResponse.getStatus());

    // 6. Insert deal
    // WebResource insertResource = webClient.resource(format("%s/data/1.0/Trade?action=create",
    // WEBAPI_BASE)); ClientResponse insertResponse =
    // insertResource.type("application/json").post(ClientResponse.class, testDeal);

    // 7. Assert no errors
    // assertEquals(format("Error inserting deal for script %s", scriptId), 200,
    // insertResponse.getStatus());

    // 8. Retrieve deal from web api database
    // Connection con =
    // DriverManager.getConnection("jdbc:h2:tcp://localhost:5055/test;IFEXISTS=TRUE;FILE_LOCK=NO",
    // "admin", "cooldude"); Statement statement = con.createStatement(); String sql = "SELECT *
    // FROM Trade WHERE TypeOfDeal = '" + scriptId + "';"; ResultSet select =
    // statement.executeQuery(sql); Assert.assertTrue("No records found in DB", select.next());
  ...
  public void testDeployment() throws Exception {
    final String input = Resources.toString(
        TomcatDeploymentITest.class.getClassLoader().getResource("data/input.json"),
        Charsets.UTF_8);
    test("TEST", input);
  }
}
